...
func TestCloudEnrichers(t *testing.T) {
	byID := enrichersByID()
	for _, spec := range Specs() {
		enrichertest.Run(t, enrichertest.Case{
			...
			Want:    enrichertest.Fact{Type: spec.FactType, Tag: "category:cloud", Name: spec.Name, Attribute: "technology", AttrValue: spec.Name},
		})
	}
}
