...
func TestSecretEnrichers(t *testing.T) {
	byID := enrichersByID()
	for _, spec := range Specs() {
		enrichertest.Run(t, enrichertest.Case{
			...
			Input: enrich.FileInput{
				RelPath:  "deploy/secrets",
				Language: spec.Languages[0],
				Source:   []byte(spec.SourceTokens[0]),
			},
			Want: enrichertest.Fact{Type: spec.FactType, Tag: "category:secrets", Name: spec.Name, Attribute: "technology", AttrValue: spec.Name},
		})
	}
}
