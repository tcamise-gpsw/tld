...
func TestRPCClientEnrichers(t *testing.T) {
	byID := enrichersByID()
	for _, spec := range Specs() {
		enrichertest.Run(t, enrichertest.Case{
			...
			Want:    enrichertest.Fact{Type: spec.FactType, Tag: "category:rpc", Name: spec.Name, Attribute: "technology", AttrValue: spec.Name},
		})
	}
}
