...
func TestStorageEnrichers(t *testing.T) {
	byID := enrichersByID()
	for _, spec := range Specs() {
		enrichertest.Run(t, enrichertest.Case{
			...
			Want:    enrichertest.Fact{Type: spec.FactType, Tag: "category:storage", Name: spec.Name, Attribute: "technology", AttrValue: spec.Name},
		})
	}
}
